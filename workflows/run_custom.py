...
def RunScanoramaIntegration(merged, integrated_sce, integrated_tsne, integrated_umap):
    ...
    script.close()
    cmd = """bsub -K -J "scanorama" -R "rusage[mem=4]" -R "select[type==CentOS7]" -W 03:00 -n 16 -o output -e error singularity exec /work/ceglian/images/scrna-r-base.img Rscript {script}""".format(script=integrate_script)
    subprocess.call(cmd.split())
