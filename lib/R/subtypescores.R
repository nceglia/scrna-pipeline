...
qdf <- data.frame(score=seurat$subtype_score1,batch=seurat$batch)
...
Idents(seurat) <- "batch"
binned <- make_hexbin(seurat, nbins=28, dimension_reduction = "umapcelltype")
dmp <- plot_hexbin_meta(binned, col="sample", action="majority")+ ggtitle("Batch") + theme(plot.title=element_text(face="bold")) + xlab("UMAP-1") + ylab("UMAP-2") 
