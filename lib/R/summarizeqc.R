...
mito <- ggplot(qdf, aes(x=mito)) + geom_histogram() + xlab('Proportion of reads in mitochondrial genes') + ylab("Cells") + geom_vline(xintercept=0.25, linetype="dashed", color = "red") + ggtitle("Mitochondrial") + theme(axis.title.x = element_text(size = 8),axis.title.y = element_text(size = 8), plot.title=element_text(face="bold"))
...
doub <- ggplot(ddf, aes(x=doublet_score)) + geom_histogram() + xlab('Doublet Score') + ylab("Cells") + geom_vline(xintercept=0.35, linetype="dashed", color = "red") + ggtitle("Doublet Scores") + theme(axis.title.x = element_text(size = 8),axis.title.y = element_text(size = 8), plot.title=element_text(face="bold"))
...
ccumap = plot_hexbin_meta(binned, col="Phase", action="majority") + ggtitle("Cell Cycle Phase") + theme(legend.title = element_blank())
...
stat <- ggtexttable(summary_stats, rows = NULL, theme = ttheme(base_style="mOrange", base_size=16))


cvm <- plotColData(sce_qc, x = "log10_total_features_by_counts", y="pct_counts_mitochondrial")  + scale_x_log10() + ggtitle("Feature vs. Percent Mito") + theme(axis.title.x = element_text(size = 13),axis.title.y = element_text(size = 13))

figure <- plot_grid(stat, mito, ribo, doub, umi, dumap, features, mitomap, ccumap, NULL, cvm, NULL, nrow = 4, ncol=3, align = "vh")

ggsave(report, figure, width=8,height=8, scale=1.5)
